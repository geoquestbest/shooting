...
$result_orders = mysqli_query($conn, "SELECT * FROM `orders_new` WHERE `status` = 2 AND `agenda` LIKE '0' AND `take_date` != '' AND `return_date` != '' LIMIT 100");
while($row_orders = mysqli_fetch_assoc($result_orders)) {
  if (strtotime($row_orders['take_date']) <= strtotime($row_orders['return_date'])) {
    ...
    $event_id = $capi->CreateCalendarEvent($calendar_id, $event_title, $full_day_event, $event_time, $user_timezone, $color_id, $data['access_token']);
    echo $row_orders['id'].". ".$event_title." ".$start_time." - ".$end_time." - ".$color_id." - ".$event_id."<br />";
    mysqli_query($conn, "UPDATE `orders_new` SET `agenda` = '$event_id' WHERE `id` = ".$row_orders['id']) or die(mysqli_error($conn));
  }
}
echo"done";