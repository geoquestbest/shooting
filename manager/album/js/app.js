...
// remove box-creation-title

function boxCreationTitle() {
    let boxCreationTitle = document.querySelector('.box-creation-title');
    let body = document.querySelector('body');
    if (boxCreationTitle) {
        body.addEventListener('click', () => {
            boxCreationTitle.style.display = 'none';
        })
    }
}
boxCreationTitle();

// creation-btn-2 active

function creationBtnTwo() {
    let creationBtnTwo = document.querySelectorAll('.creation-btn-2');
    if (creationBtnTwo) {
        creationBtnTwo.forEach((item) => {
            item.addEventListener('click', function () {
                for (let i = 0; i < creationBtnTwo.length; i++) {
                    if (creationBtnTwo[i].classList.contains('active')) {
                        if (!item.classList.contains('no-active')) {
                            creationBtnTwo[i].classList.remove('active');
                        }
