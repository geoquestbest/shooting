const TextureFactory = ( { domContainerID, width, height, source, mask, orientation, quantity } ) =>
{
	...
	const toJSON = () => 
	{
		...
		_objects.selectAll( 'g[class=zone]' ).forEach( element => 
		{
			...
			let object = {};
				...
				object.matrix = element.transform().localMatrix.toString();
	...
	const addZone = () => 
	{
		if( canAddZone() )
		{
			...
			let rect = element.rect( 0, 0, w, h );
				rect.attr( { fill:getHex( ZONE_COLORS[ _zonesQty ] ) } );
				
			let text = element.text( w / 2, h / 2, ( _zonesQty + 1 ).toString() );
				text.attr
				( {
					textAnchor:'middle',
					fontWeight:'bold',
			...
			element.transform( new Snap.Matrix().scale( rectScale ) );
