...
$selected_options_arr2 = explode(",", str_replace(",Livraison", "", str_replace(",Retrait boutique", "", str_replace(", ", ",", trim($row_orders['selected_options'])))));
  ...
  if (in_array($value['name'], $selected_options_arr)) {
    for($i = 0; $i < count($selected_options_arr); $i++) {
      if (trim($selected_options_arr[$i]) == $value['name']) {
        $current_option = explode(":", $selected_options_arr2[$i]);
        if ($current_option[2] != $value['price']) {
          if ($options_txt != "") {
            $options_txt .= ", ";
          }
          $options_txt .=  $value['name']." - Option price: ".$value['price']." / Order price: ".$current_option[2];
        }
      }
    }
  }
...
if (($price != $row_orders['price'] && $row_orders['price'] != 0) || $options_txt != '') {
  echo "ID ".$row_orders['id'].". ";
  if ($price != $row_orders['price'] && $row_orders['price'] != 0) {
   echo"Box price: ".$price." / Order price: ".$row_orders['price']." | ";
  }
  echo $options_txt."<br />";
}
