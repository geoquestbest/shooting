window.onload = () =>
{
    ...
    var url_string = window.location.href,
      ...
      param_template_id = url.searchParams.get("template_id"),
      param_json = url.searchParams.get("json"),
      param_vegas = url.searchParams.get("vegas");
	...
	const loadFonts = ( onComplete ) =>
	{
		...
		[
			...
			},
			{
				...
				fonts:
				[
					{ url:'fonts/Make-the-Best/Make-the-Best.otf', 	style:'normal', weight:'400', type:'font/opentype' },
				],
			},
			{
				name:'Christmas Bell',
				...
				[
					{ url:'fonts/Christmas Bell/ChristmasBell.ttf', 	style:'normal', weight:'400', type:'application/x-font-ttf' },
                        ...
                        {
				name:'Nickainley-Normal',
				fonts:
				[
					{ url:'fonts/Nickainley-Normal/Nickainley-Normal.otf', 	style:'normal', weight:'400', type:'font/opentype' },
				],
			},
			{
				name:'Christmas Day',
            ...
            {
				// NOTE: 'Sugar & Vinegar' : [ FireFox bug -> 'An invalid or illegal string was specified' ]
				name:'Sugar and Vinegar',
				fonts:
				[
					{ url:'fonts/Sugar & Vinegar/Sugar & Vinegar.ttf', 	style:'normal', weight:'400', type:'application/x-font-ttf' },
				],
			},
			{
				name:'Din',
				...
				fonts:
				[
					{ url:'fonts/Sweet Pea/Sweet Pea.ttf', 	style:'normal', weight:'400', type:'application/x-font-ttf' },
				],
			},
			{
				name:'Free Style',
		...
		const _onComplete = () =>
		{
			for( let i = fontFamilyList.length - 1; i >= 0 ; i-- )
			{
				let family = fontFamilyList[ i ];

				for( let f = family.fonts.length - 1; f >= 0 ; f-- )
				{
					let data = family.fonts[ f ];

					if( data.font == null )
					{
						console.log( 'delete font -> ' + data.url );
						family.fonts.splice( f, 1 );
					}
				}

				if( family.fonts.length == 0 )
				{
					console.log( 'delete family -> ' + family.name );
					fontFamilyList.splice( i, 1 );
				}
			}
					...
					response.arrayBuffer().then( buffer =>
					{
						const reader = new FileReader();
							  reader.onload = () =>
							  {
									...
									let ff = new FontFace( family.name, 'url("' + data.dataURL + '")', { style:data.style, weight:data.weight } );
										ff.load().then
										(
											font =>
											{
							  ...
							  reader.onerror = () => _onError( reader.error );
	...
	const UI = ( { textureFactory, fontFamilyList } ) =>
	{
		...
		const _disableBackgroundButton = document.querySelector( '#voir' );
			  _disableBackgroundButton.onclick = () => textureFactory.setBackgroundEnabled( !_disableBackgroundButton.checked );
			  _disableBackgroundButton.onclick();
    ...
    const _saveAsImagetoServerButton = document.querySelector( '#saveAsImagetoServerButton' );
      if( _saveAsImagetoServerButton )
		...
		textureFactory.signals.zoomChanged.add( zoom => _zoomInputRange.value = zoom );
		...
		const _textAnchorRadiogroup = [];

		// ----------------------------------------------------------------------
		// Radiogroup
		// ----------------------------------------------------------------------

			document.querySelectorAll( 'input[name=textAnchor]' ).forEach( radio => _textAnchorRadiogroup.push( radio ) );
		...
		// ----------------------------------------------------------------------
			...
			const getSelectedTextAnchor = () =>
			{
				let result = '';

				_textAnchorRadiogroup.forEach( radio =>
				{
					if( radio.checked )
						result = radio.value;
				} );
			...
			const setSelectedTextAnchor = ( value ) =>
			{
				_textAnchorRadiogroup.forEach( radio =>
				{
		...
		textureFactory.signals.elementSelected.add( uuid =>
		{
			...
			setSelectedTextAnchor( attributes.textAnchor );

			_textAnchorRadiogroup.forEach( radio =>
			{
				radio.onchange = () => textureFactory.setElementAttributes( uuid, { textAnchor:getSelectedTextAnchor() } );
			} );
			...
			const onTextAttributesChanged = ( changedUUID, changedAttributes ) =>
			{
				if( uuid == changedUUID )
				{
					...
					if( changedAttributes.hasOwnProperty( 'textAnchor' ) )
						setSelectedTextAnchor( changedAttributes.textAnchor );
			...
			const onTextDeselected = deselectedUUID =>
			{
				if( uuid == deselectedUUID )
				{
					...
					_textAnchorRadiogroup.forEach( radio => radio.onchange = null );
			...
			loadFonts( fontFamilyList =>
			{
				...
				ui.signals.saveAsImagetoServerRequested.add( () =>
				{
											...
											if( request.readyState === 4 && request.status === 200 )
											{
												...
												setTimeout(() =>
												{
													...
													if (+param_order == 0) {
													  ...
													  window.location.href = 'https://ftp.shootnbox.fr/template/valiadation.php';
													}
												}, 1000);
											}
										} );
					...
					else
					{
						...
						//textDialogContainer.classList.add( 'active' );
						//textDialog.classList.add( 'active' );
						openPopup('.popup-valider');

						const closeButton = textDialog.querySelector( '.dialog-action-cancel' );
						const saveButton = document.querySelector( '.dialog-action-save' );
						...
						const saveDialog = () =>
						{
							...
							textureFactory.getResultDataURL( dataURL =>
							{
								console.log( 'textureFactory.getResultDataURL [ dataURLLength:' + ( dataURL ? dataURL.length : 0 ) + ' ]' );

								const request = new XMLHttpRequest();
								const url = "../d26386b04e.php";
								const params = "event=save_image" + "&data=" + dataURL + '&order_id=' + param_order + '&template_id=' + param_template_id  + '&vegas=' + param_vegas;;
									...
									request.addEventListener("readystatechange", () =>
									{
										if(request.readyState === 4 && request.status === 200)
										{
											...
											setTimeout(() =>
											{
												if (+param_order == 0) {
													window.location.href = '../templates_list.php';
												} else if (+param_order < 0) {
													window.location.href = '../ipad_vegas_list.php';
													if (+param_vegas != 1) {
														window.location.href = '../ipad_vegas_list.php';
													} else {
