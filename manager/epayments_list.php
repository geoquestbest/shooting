<?php
	switch($_GET['status']) {
		default: $page_title = "Entreprise paiements"; break;
	}
	include("header.php");
	if (isset($_GET['status'])) {
    switch ($_GET['status']) {
      case 0:
        $rq = "WHERE `status` > 0 AND `select_type` LIKE '%entreprise%' AND `deposit` = 0 AND `payment_status` = 0";
      break;
      case 1:
        $rq = "WHERE `status` > 0 AND `select_type` LIKE '%entreprise%' AND `deposit` > 0 AND `payment_status` = 0";
      break;
      default:
         $rq = "WHERE `status` > 0 AND `select_type` LIKE '%entreprise%' AND `payment_status` > 0";
      break;
    }
	} else {
		$rq = "WHERE `status` > 0 ";
  ...
  if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
    ...
    $start_date = date("d.m.Y", strtotime("01.01.".(date("Y", time()) - 1)));
          ...
          if (strtotime($row_orders['event_date']) >= strtotime($start_date) && strtotime($row_orders['event_date']) <= strtotime($end_date) && (($_GET['facture_date'] == 1 && mysqli_num_rows($result_facture ) > 0) || (!isset($_GET['facture_date']) || $_GET['facture_date'] == 0))) {
            $delivery = (mb_strpos($row_orders['selected_options'], 'Retrait boutique') || $row_orders['delivery_options'] == "") ? 'Retrait boutique' : 'Livraison';
						echo'<tr id="tr'.$row_orders['id'].'" class="gradeX';if ($i%2 == 0) {echo' odd';} else {echo' even';} echo'">
							<td><a href="edit_order.php?order_id='.$row_orders['id'].'" target="_blank">'.$row_orders['id'].'</a></td>
              <td>
                '.$row_orders['societe'].'
              </td>
              <td>
              ...
              <td class="text-center">';
                ...
                if (mysqli_num_rows($result_facture ) > 0) {
                  ...
                  <td class="text-center" data-sort="'.strtotime($row_orders['event_date']).'">'.$row_orders['event_date'];
                } else {
              ...
              <td class="text-center">
                '.$row_orders['sage'].'
							...
							<td data-order="'.strtotime($row_orders['event_date']).'">'.$row_orders['event_date'].'</td>
              ...
              if ($_GET['status'] == 1) {
                echo'<td class="text-center">'.number_format($row_orders['total']*1.2 - $row_orders['deposit'], 2).'</td>';
              }
							echo'<td class="text-center"><input type="text" value="'.$row_orders['comment_payment'].'" class="comment_payment form-control" data-id="'.$row_orders['id'].'" style="width: 240px;" /></td>
              <td class="text-center">';
                  echo'<a href="to_pdf.php?order_id='.$row_orders['id'].'" class="btn btn-success btn-icon btn-circle btn-sm" title="Facture" target="_blank"><i class="fa fa-list"></i></a>';
               echo'</td>
              <td>';
                ...
                echo'<a href="#" class="btn '.($row_orders['payment_status'] != 2 ? 'btn-danger' : 'btn-success').' btn-icon btn-circle btn-sm pay-status" data-id="'.$row_orders['id'].'" data-btn="2" data-status="'.$row_orders['payment_status'].'" title="Solde statut"><i class="fa fa-money"></i></a>&nbsp;';
								...
								echo'<a href="edit_epayment.php?order_id='.$row_orders['id'].'&status='.mysqli_real_escape_string($conn, $_GET['status']).'" class="btn btn-warning btn-icon btn-circle btn-sm" title="Modifier"><i class="fa fa-edit"></i></a>';

                if ($row_orders['payment_status'] != 2) {
                  //echo'&nbsp;<a href="javascript:void(0)" class="btn btn-success btn-icon btn-circle btn-sm" title="Soldé" onClick="payOrder('.$row_orders['id'].')"><i class="fa fa-money"></i></a>&nbsp;';
                }
							echo'</td>
						</tr>';
						$i++;
            $total = $total + $row_orders['total'];
            $total2 = $total2 + $row_orders['deposit'];
            $total3 = $total3 + ($row_orders['total'] - $row_orders['deposit']);
					}
				}
				?>
			</tbody>
...
<script>
    ...
    $('.show').on('click', function(event) {
      ...
      if ($('.start_date').val() != '' && $('.end_date').val() != '') {
        window.location.href = 'epayments_list2.php?status=<?php echo $_GET['status'] ?>&start_date=' + $('.start_date').val() + '&end_date=' + $('.end_date').val() + '&facture_date=' + ($('.facture_date').is(':checked') ? 1 : 0) + '<?php echo((isset($_GET['facteur']) && $_GET['facteur'] == 1) ? '&facteur=1' : '')?>';
      }
    });
      ...
      fixedHeader:           {
        ...
        footer: false
				...
				<?php if (isset($_GET['facteur']) && $_GET['facteur'] == 1) { ?>
          {
            ...
            action: function (e, dt, node, config) {
              window.location.href = 'factor_xls.php?orders_ids=<?php echo $orders_ids?>';
            }
          },
        <?php
          } else {
        ...
        ?>
        <?php if (isset($_GET['facteur']) && $_GET['facteur'] == 1 && $_GET['status'] == 0) { ?>
        {
          text: '<i class="fa fa-download"></i> Exporter',
          action: function (e, dt, node, config) {
            window.open('multi_pdf.php?orders_ids=<?php echo $orders_ids?>');
          }
        },
        <?php } ?>
				{
  ...
  $('.comment_payment').on('blur', function(event) {
      ...
      $.ajax({
        type: 'POST',
        url: 'd26386b04e.php',
        data: {event: 'comment_payment', id: order_id, text: text},
        cache: false,
  ...
  $('.pay-status').on('click', function(event) {
    ...
    if ($(this).hasClass('btn-danger')) {
      ...
      var status = $(this).attr('data-status') == 2 ? 0 : $(this).attr('data-status');
    }
    var order_id = $(this).attr('data-id');
  ...
  function payOrder(id) {
    swal({
      title: 'Volez vous,',
      text: 'passer cette resrvation en soldé ?',
      type: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
