<?php
    ...
    if ($_GET['status'] > 0) {
      ...
      $rq = "WHERE `status` > 0 AND `select_type` LIKE '%entreprise%' AND  `payment_status` = ".mysqli_real_escape_string($conn, $_GET['status']);
    }
	} else {
		$rq = "WHERE `status` > 0 ";
	}
    ...
    <form class="form-horizontal" style="position: absolute; left: 45vw;">
      <div class="form-group">
		...
		<table id="data-table" class="table table-striped table-bordered orders-table nowrap" width="100%">
          ...
          <th>Tarif TTC, €</th>
          <th>Comment</th>
          <th class="no-sort">Enlèvement</th>
          <th class="no-sort">Retour</th>
          ...
          $result_facture = mysqli_query($conn, "SELECT * FROM `facture` WHERE `order_id` = ".$row_orders['id']." ".$rq2." ORDER BY `id` DESC LIMIT 1");
              ...
              <td class="text-center">
                '.$row_orders['sage'].'
							...
							<td>'.$delivery.'</td>
							<!--td class="text-center">'.($row_orders['total']*1.2).( $row_orders['discount'] ? '<br />Votre reduction avec code promo : -'.$row_orders['discount'].'€<br />' : '').'</td-->
              ...
              <td class="text-center">
                '.$row_orders['take_date'].'
              </td>
              <td class="text-center">
                '.$row_orders['return_date'].'
              </td>
              <td class="text-center"><input type="text" value="'.$row_orders['comment_payment'].'" class="comment_payment form-control" data-id="'.$row_orders['id'].'" style="width: 240px;" /></td>
              <td class="text-center">';
...
<script>
    ...
    $('.show').on('click', function(event) {
      ...
      if ($('.start_date').val() != '' && $('.end_date').val() != '') {
        window.location.href = 'epayments_list.php?status=<?php echo $_GET['status'] ?>&start_date=' + $('.start_date').val() + '&end_date=' + $('.end_date').val() + '&facture_date=' + ($('.facture_date').is(':checked') ? 1 : 0);
      }
    });
