              ...
              <td>';
              	if ($row_orders['payment_type2'] != 0) {
                	echo'<a href="#" class="btn '.($row_orders['payment_status'] == 0 ? 'btn-danger' : 'btn-success').' btn-icon btn-circle btn-sm pay-status" data-id="'.$row_orders['id'].'" title="Acompte statut"><i class="fa fa-money"></i></a>&nbsp;';
								...
								echo'<a href="'.(isset($_GET['status']) ? 'edit_epayment.php?order_id='.$row_orders['id'].'&status='.mysqli_real_escape_string($conn, $_GET['status']) : 'edit_epayment2.php?order_id='.$row_orders['id']).'" class="btn btn-warning btn-icon btn-circle btn-sm" title="Modifier"><i class="fa fa-edit"></i></a>
							</td>
						</tr>';
						$i++;
