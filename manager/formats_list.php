...
<div class="panel panel-inverse">
	<div class="panel-heading">
		...
		<h4 class="panel-title"><?php echo QUESTS_TYPES ?></h4>
	</div>
		...
		<table id="data-table" class="table table-striped table-bordered nowrap types-table" width="100%">
			...
			<tbody>
				...
				while($row_types = mysqli_fetch_assoc($result_types)) {
					echo'<tr id="tr'.$row_types['id'].'" class="'; if ($i%2==0) {echo'odd';} else {echo'even';} echo' gradeX">
						<td>'.$row_types['id'].'</td>
						<td>'.$row_types['title'].'</td>
						<td>
							<a href="edit_type.php?type_id='.$row_types['id'].'" class="btn btn-warning btn-icon btn-circle btn-sm" title="'.EDIT.'"><i class="fa fa-edit"></i></a>
							<a class="btn btn-danger btn-icon btn-circle btn-sm project-type-delete" title="'.DELETE.'" onClick="deleteType('.$row_types['id'].');"><i class="fa fa-times"></i></a>
						</td>
					</tr>';
					$i++;
				}
