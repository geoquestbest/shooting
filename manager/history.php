      ...
      <div class="form-group">
		...
		<table id="data-table" class="table table-striped table-bordered nowrap widgets-table" width="100%">
          ...
          <th style="width: auto;">Utilisateur</th>
					<th style="width: auto;">Factures</th>
          <th style="width: auto;">Date de facture</th>
          <th style="width: auto;">Date de l’évènement</th>
				</tr>
			</thead>
			<tbody>
				<?php
				$result_facture = mysqli_query($conn, "SELECT DISTINCT(order_id) FROM `facture` ".$rq." ORDER BY `order_id` DESC");
				$i = 1;
				while($row_facture = mysqli_fetch_assoc($result_facture)) {
					$result_orders = mysqli_query($conn, "SELECT * FROM `orders_new` WHERE `id` = ".$row_facture['order_id']);
          if (mysqli_num_rows($result_orders) > 0) {
  					$row_orders = mysqli_fetch_assoc($result_orders);
            if ($_GET['facture_date'] == 1 || ((!isset($_GET['facture_date']) || $_GET['facture_date'] == 0) && strtotime($row_orders['event_date']) >= strtotime($start_date) && strtotime($row_orders['event_date']) <= strtotime($end_date))) {
              $result_facture2 = mysqli_query($conn, "SELECT * FROM `facture` WHERE `order_id` = ".$row_facture['order_id']." ORDER BY `id`");
                $j = 1;
                while($row_facture2 = mysqli_fetch_assoc($result_facture2)) {
                  if ($j < mysqli_num_rows($result_facture2)) {
                    echo'<tr class="'; if ($i%2==0) {echo'odd';} else {echo'even';} echo' gradeX">
           						<td class="text-center">'.$row_facture['order_id'].'</td>
                      <td class="text-left">
                        ...
                        <a class="facture-list" data-url="'.$row_orders['num_id'].'/'.$row_facture2['pdf'].'" href="../uploads/Factures/'.$row_orders['num_id'].'/'.$row_facture2['pdf'].'" target="_blank">'.$row_facture2['pdf'].'</a>
                      </td>
                      <td class="text-center" data-sort="'.$row_facture2['created_at'].'">'.date("d.m.Y H:i", $row_facture2['created_at']).'</td>
...
<script>
    ...
    $('.show').on('click', function(event) {
      ...
      if ($('.start_date').val() != '' && $('.end_date').val() != '') {
        window.location.href = 'history.php?start_date=' + $('.start_date').val() + '&end_date=' + $('.end_date').val() + '&facture_date=' + ($('.facture_date').is(':checked') ? 1 : 0);
      }
    });
