...
<div class="panel panel-inverse">
	...
	<div class="panel-body">
		<table id="data-table" class="table table-striped table-bordered orders-table nowrap" width="100%">
			...
			<tbody>
				<?php
				$result_orders = mysqli_query($conn, "SELECT * FROM `orders_new` WHERE (`box_type` LIKE 'ring') AND (`image` != ''/*OR `select_type` LIKE 'Une entreprise'*/) ORDER BY `id` DESC");
				$i=1;
               ...
               echo'</td>
	              ...
	              echo'<a href="be/?image='.$row_orders['image'].'&order_id='.$row_orders['id'].'&data='.$row_orders['data'].'" class="btn btn-'.(($row_orders['data'] == "") ? 'success' : 'danger').' btn-icon btn-circle btn-lg" title="Configuration 2"><i class="fa fa-edit"></i></a>';}
	              echo'</td>
							<td>';
