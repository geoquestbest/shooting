...
<div class="panel panel-inverse">
	...
	<div class="panel-body">
		<table id="data-table" class="table table-striped table-bordered orders-table nowrap" width="100%">
			...
			<tbody>
				<?php
				$result_orders = mysqli_query($conn, "SELECT * FROM `orders_new` WHERE `box_type` LIKE '%vegas%' AND `status` = 2 ORDER BY `id` DESC");
				$i=1;
				while($row_orders = mysqli_fetch_assoc($result_orders)) {
					if (strtotime($row_orders['event_date']) >= strtotime(date("d.m.Y", time())."00:00:00")) {
						$add = true;

						if ($row_orders['image_vegas'] != "") {
							$template = ADMIN_UPLOAD_IMAGES_DIR.$row_orders['image_vegas'];
						} else {
	            ...
	            $template = ADMIN_UPLOAD_IMAGES_DIR.$row_gallery_videos['video'];
	          }
		        ...
		        if ($add) {
	               ...
	               echo'</td>
								...
								<td>';
									echo'<a class="fancybox" href="'.$template.'"><img src="'.$template.'" style="width: 120px" alt="" /></a>';
		              ...
		              if (strtolower($row_orders['image_vegas']) != "") {
		              	echo'<a href="be/?image='.$row_orders['image_vegas'].'&order_id='.$row_orders['id'].'&data='.$row_orders['data'].'" class="btn btn-'.(($row_orders['data'] == "") ? 'success' : 'danger').' btn-icon btn-circle btn-lg" title="Configuration 2"><i class="fa fa-edit"></i></a>
		              	<a href="./editor/?image='.$row_orders['image_vegas'].'&sample_image='.$row_orders['image_vegas'].'&json=&order_id=-'.$row_orders['id'].'&template_id=0&vegas=1" class="btn btn-inverse btn-icon btn-circle btn-lg" title="Éditeur" data-id="'.$row_templates['id'].'"><i class="fa fa-edit"></i></a>';
		             	}
		              echo'</td>
								<td>';
									if ($row_orders['image'] != "" && $row_orders['template_status'] == 0) {
									...
									echo'<a href="edit_order.php?order_id='.$row_orders['id'].'&status='.mysqli_real_escape_string($conn, $_GET['status']).'&vegas=ipad&back_url=ipad_vegas_list.php?status=1" class="btn btn-warning btn-icon btn-circle btn-sm" title="Modifier"><i class="fa fa-edit"></i></a>
