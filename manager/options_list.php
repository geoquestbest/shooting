...
<div class="panel panel-inverse">
	<div class="panel-heading">
		...
		<h4 class="panel-title">Options de bornes</h4>
	</div>
          ...
          <th style="width: 100px;">Entreprise prix HT, €</th>
			...
			<tbody>
				...
				while($row_options = mysqli_fetch_assoc($result_options)) {
					echo'<tr id="tr'.$row_options['id'].'" class="'; if ($i%2==0) {echo'odd';} else {echo'even';} echo' gradeX">
						<td class="text-center">'.$row_options['id'].'</td>
						<td>';
  						if ($row_options['image'] != "") {
  							echo'<a class="fancybox" href="'.ADMIN_UPLOAD_IMAGES_DIR.$row_options['image'].'" title="'.$row_options['title'].'"><img src="'.ADMIN_UPLOAD_IMAGES_DIR.getImageUrl($row_options['image'], '120').'" alt="" /></a>';
  						}
  					echo'</td>
						<td class="text-center">'.$row_options['icon'].'</td>
						<td>'.$row_options['title'].'</td>
            <td class="text-center">'.$row_options['price'].'</td>
            <td class="text-center">'.$row_options['eprice'].'</td>
						<td class="text-center">';
							if ($row_options['status'] == 1) {
								echo'<a class="btn btn-success btn-icon btn-circle btn-sm" title="Désactiver" onClick="activate_deactivate(this, '.$row_options['id'].');"><i class="fa fa-eye"></i></a>';
							} else {
								echo'<a class="btn btn-default btn-icon btn-circle btn-sm" title="Activer" onClick="activate_deactivate(this, '.$row_options['id'].');"><i class="fa fa-eye-slash"></i></a>';
							}
							echo'&nbsp;<a href="edit_option.php?option_id='.$row_options['id'].'" class="btn btn-warning btn-icon btn-circle btn-sm" title="'.EDIT.'"><i class="fa fa-edit"></i></a>
							<a class="btn btn-danger btn-icon btn-circle btn-sm project-type-delete" title="'.DELETE.'" onClick="deleteOption('.$row_options['id'].');"><i class="fa fa-times"></i></a>
						</td>
					</tr>';
					$i++;
				}
			...
			buttons: [
				'csv', 'excel', 'pdf', 'print',
				{
					...
					action: function (e, dt, node, config) {
						window.location.href = 'add_option.php';
					}
				}
			],
			oLanguage: {
		...
		swal({
			...
			if (data.value) {
				$.ajax({
					type: 'POST',
					url: 'd26386b04e.php',
					data: {event: 'delete_option', id: id},
					cache: false,
	...
	function activate_deactivate(ele, id) {
		...
		if ($(ele).hasClass('btn-default')) {
			...
			$.ajax({
				type: 'POST',
				url: 'd26386b04e.php',
				data: {event: 'option_status', id: id, status: 1},
				cache: false
			});
		} else {
			...
			$.ajax({
				type: 'POST',
				url: 'd26386b04e.php',
				data: {event: 'option_status', id: id, status: 0},
				cache: false
			});
		}
	};
