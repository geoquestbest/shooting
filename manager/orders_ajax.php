...
if (isset($_GET['arch'])) {
  $rq .= ($rq != "" ? " AND STR_TO_DATE(`event_date`, '%d.%m.%Y') <= (CURDATE() + INTERVAL 1 DAY)" : "WHERE STR_TO_DATE(`event_date`, '%d.%m.%Y') <= (CURDATE() + INTERVAL 1 DAY)");
} else {
  $rq .= ($rq != "" ? " AND STR_TO_DATE(`event_date`, '%d.%m.%Y') >= CURDATE()" : "WHERE STR_TO_DATE(`event_date`, '%d.%m.%Y') >= CURDATE()");
}
...
if ($_GET['search_type'] == 1) {
  ...
  $rq .= ($rq != "" ? " AND UNIX_TIMESTAMP(STR_TO_DATE(`event_date`, '%d.%m.%Y')) >= ".strtotime($start_date." 00:00:00")." AND UNIX_TIMESTAMP(STR_TO_DATE(`event_date`, '%d.%m.%Y')) <= ".strtotime($end_date." 23:59:59") : "WHERE UNIX_TIMESTAMP(STR_TO_DATE(`event_date`, '%d.%m.%Y')) >= ".strtotime($start_date." 00:00:00")." AND UNIX_TIMESTAMP(STR_TO_DATE(`event_date`, '%d.%m.%Y')) <= ".strtotime($end_date." 23:59:59"));
}
...
//file_put_contents('rq.txt', "SELECT * FROM `orders_new` ".$rq);
...
while($row_orders = mysqli_fetch_assoc($result_orders)) {
  /*f (((!isset($_GET['arch']) && floor((strtotime($row_orders['event_date'])  + 24*3600 - time())/(3600*24)) >= 0 && strtotime($row_orders['event_date']) >= strtotime($start_date) && strtotime($row_orders['event_date']) <= strtotime($end_date)) && (!isset($_GET['search_type']) || $_GET['search_type'] == 0) || (isset($_GET['arch']) && floor((strtotime($row_orders['event_date']) + 24*3600 - time())/(3600*24)) < 0) && strtotime($row_orders['event_date']) >= strtotime($start_date) && strtotime($row_orders['event_date']) <= strtotime($end_date) && (!isset($_GET['search_type']) || $_GET['search_type'] == 0)) || ($_GET['search_type'] == 1 && !isset($_GET['arch']) && floor((strtotime($row_orders['event_date'])  + 24*3600 - time())/(3600*24)) >= 0) || ($_GET['search_type'] == 1 && isset($_GET['arch']) && floor((strtotime($row_orders['event_date']) + 24*3600 - time())/(3600*24)) < 0)) {*/
    ...
    $data[] = array(
      ...
      'devis' => $row_orders['status'] == 0 ? ($row_orders['devis'] != 0 ? '<a href="to_pdf.php?order_id='.$row_orders['id'].'&devis='.$row_orders['devis'].'" title="Devis" target="_blank">'."DE".$row_orders['devis'].'</a><a href="#" class="btn '.($row_orders['to_mail'] == 0 ? 'btn-default' : 'btn-success').' btn-icon btn-circle to-mail" data-id="'.$row_orders['id'].'" title="Mail"><i class="fa fa-envelope-o"></i></a>' : '') : ($row_orders['devis'] != 0 ? '<a href="to_pdf.php?order_id='.$row_orders['id'].'&devis='.$row_orders['devis'].'" title="Devis" target="_blank">'."DE".$row_orders['devis'].'</a>' : ''),
      ...
      'facture' => '<a href="to_pdf.php?order_id='.$row_orders['id'].'" title="Facture" target="_blank">'.$row_orders['num_id'].'</a>'.($row_orders['status'] == 2 ? '<br /><a href="to_pdf.php?order_id='.$row_orders['id'].'&refund=true" class="btn btn-danger btn-icon btn-circle btn-sm" title="Remboursement" target="_blank"><i class="fa fa-close"></i></a><br /><input type="checkbox" class="facture_enabled'.$row_orders['id'].'" '.($row_orders['facture_enabled'] == 1 ? ' checked' : '').' onChange="factureEnabled('.$row_orders['id'].')" />' : ''),
      'refuse' => $refuse,
      'actions' => $actions
    );
