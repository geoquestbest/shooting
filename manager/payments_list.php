<?php
	switch($_GET['status']) {
		default: $page_title = "Particulier paiements"; break;
	}
	include("header.php");
...
<script>
    ...
    $('.show').on('click', function(event) {
      ...
      if ($('.start_date').val() != '' && $('.end_date').val() != '') {
        window.location.href = 'payments_list.php?status=<?php echo $_GET['status'] ?>&start_date=' + $('.start_date').val() + '&end_date=' + $('.end_date').val() + '&facture_date=' + ($('.facture_date').is(':checked') ? 1 : 0);
      }
    });
