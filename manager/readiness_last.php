		...
		<table id="data-table" class="table table-striped table-bordered orders-table nowrap" width="100%">
          ...
          $result_configure_orders = mysqli_query($conn, "SELECT * FROM `configure_orders` WHERE `order_id` = ".$row_orders['id']);
					...
					echo'<tr id="tr'.$row_orders['id'].'" class="gradeX';if ($i%2 == 0) {echo' odd';} else {echo' even';} echo'">
						<td data-sort="'.strtotime($row_orders['event_date']).'">'.$row_orders['event_date'].'<br />'.$row_orders['id'].'</td>
            ...
            echo'<td class="text-center">
              <select class="courier form-control" data-id="'.$row_orders['id'].'" data-km="'.($data_km/2).'" style="width: 120px;">
                ...
                while($row_delivery = mysqli_fetch_assoc($result_delivery)) {
                  echo'<option value="'.$row_delivery['title'].'"'.($row_orders['courier'] == $row_delivery['title'] ? ' selected' : '').'>'.$row_delivery['title'].'</option>';
                }
              echo'</select>
            </td>
            ...
            <td class="text-center">
              <select class="courier_r form-control" data-id="'.$row_orders['id'].'" data-km="'.($data_km/2).'" style="width: 120px;">
                ...
                $result_delivery = mysqli_query($conn, "SELECT * FROM `delivery`");
                while($row_delivery = mysqli_fetch_assoc($result_delivery)) {
                  echo'<option value="'.$row_delivery['title'].'"'.($row_orders['courier_r'] == $row_delivery['title'] ? ' selected' : '').'>'.$row_delivery['title'].'</option>';
                }
              echo'</select>
            </td>
            ...
            <td class="text-center"><input type="text" value="'.$row_orders['comment'].'" class="comment form-control" data-id="'.$row_orders['id'].'" style="width: 240px;" /></td>';
            if (!isset($_GET['arch'])) {
              echo'<td class="text-center">
