<?php
	...
	$page_title = "Récuperation pour les bornes";
	include("header.php");
?>
<!-- begin panel -->
<div class="panel panel-inverse">
	<div class="panel-heading">
		...
		<h4 class="panel-title">Récuperation pour les bornes</h4>
	</div>
		...
		<table id="data-table" class="table table-striped table-bordered nowrap types-table" width="100%">
			<thead>
				<tr>
					...
					<th>Récuperation pour les bornes</th>
          ...
          <th style="width: 100px;">Entreprise prix HT, €</th>
			...
			<tbody>
				...
				while($row_recovery = mysqli_fetch_assoc($result_recovery)) {
					echo'<tr id="tr'.$row_recovery['id'].'" class="'; if ($i%2==0) {echo'odd';} else {echo'even';} echo' gradeX">
						<td class="text-center">'.$row_recovery['id'].'</td>
						<td>';
  						if ($row_recovery['image'] != "") {
  							echo'<a class="fancybox" href="'.ADMIN_UPLOAD_IMAGES_DIR.$row_recovery['image'].'" title="'.$row_recovery['title'].'"><img src="'.ADMIN_UPLOAD_IMAGES_DIR.getImageUrl($row_recovery['image'], '120').'" alt="" /></a>';
  						}
  					echo'</td>
						<td>'.$row_recovery['title'].'</td>
            <td class="text-center">'.$row_recovery['price'].'</td>
            <td class="text-center">'.$row_recovery['eprice'].'</td>
						<td class="text-center">';
							if ($row_recovery['status'] == 1) {
								echo'<a class="btn btn-success btn-icon btn-circle btn-sm" title="Désactiver" onClick="activate_deactivate(this, '.$row_recovery['id'].');"><i class="fa fa-eye"></i></a>';
							} else {
								echo'<a class="btn btn-default btn-icon btn-circle btn-sm" title="Activer" onClick="activate_deactivate(this, '.$row_recovery['id'].');"><i class="fa fa-eye-slash"></i></a>';
							}
							echo'&nbsp;<a href="edit_recovery.php?recovery_id='.$row_recovery['id'].'" class="btn btn-warning btn-icon btn-circle btn-sm" title="'.EDIT.'"><i class="fa fa-edit"></i></a>
							<a class="btn btn-danger btn-icon btn-circle btn-sm project-type-delete" title="'.DELETE.'" onClick="deleteOption('.$row_recovery['id'].');"><i class="fa fa-times"></i></a>
						</td>
					</tr>';
					$i++;
				}
			...
			buttons: [
				'csv', 'excel', 'pdf', 'print',
				{
					...
					action: function (e, dt, node, config) {
						window.location.href = 'add_recovery.php';
					}
				}
			],
			oLanguage: {
		...
		swal({
			...
			if (data.value) {
				$.ajax({
					type: 'POST',
					url: 'd26386b04e.php',
					data: {event: 'delete_recovery', id: id},
					cache: false,
	...
	function activate_deactivate(ele, id) {
		...
		if ($(ele).hasClass('btn-default')) {
			...
			$.ajax({
				type: 'POST',
				url: 'd26386b04e.php',
				data: {event: 'recovery_status', id: id, status: 1},
				cache: false
			});
		} else {
			...
			$.ajax({
				type: 'POST',
				url: 'd26386b04e.php',
				data: {event: 'recovery_status', id: id, status: 0},
				cache: false
			});
		}
	};
