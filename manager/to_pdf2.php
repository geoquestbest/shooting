...
foreach ($options as $key => $value) {
  if (in_array($value['name'], $selected_options_arr)) {
    if (mb_strtolower($value['name']) == 'assurance dégradation') {
      $assurance = true;
    }
...
foreach ($deliverys as $key => $value) {
  if (in_array($value['name'], $delivery_options_arr)) {
    if (strpos(mb_strtolower($value['name']), 'livraison') !== false || strpos(mb_strtolower($value['name']), 'installation') !== false) {
      $livriason = true;
    }
...
if (strpos(strtolower($row_orders['select_type']), 'entrepris') === false) {
  $pdf->SetX(29);
  $y = $pdf->GetY();
  $pdf->MultiCell(60, 3, mb_convert_encoding("ACOMPTE : ".($row_orders['deposit'] == 0 ? $row_orders['advance_payment'] : $row_orders['deposit'])."€\nSOLDE ".($livriason ? "avant livriason": "au retrait")." : ".(($total_ht - $total_ht/100*$row_orders['transportation_time']) + ($total_tva - $total_tva/100*$row_orders['transportation_time']) - ($row_orders['deposit'] == 0 ? $row_orders['advance_payment'] : $row_orders['deposit']))."€", 'windows-1252'), 'L', 'L', 0);
  $pdf->SetX(29);
...
$pdf->SetX(100);
if ($row_orders['deposit'] == 0 && $row_orders['advance_payment'] != 0 && $row_orders['status'] != 0) {
  $pdf->Cell(85, $step, mb_convert_encoding('Acompte', 'windows-1252'), 'L', 0, 'L', 0);
  $pdf->Cell(15, $step, mb_convert_encoding(number_format(($row_orders['deposit'] == 0 ? $row_orders['advance_payment'] : $row_orders['deposit']), 2, ',', ''), 'windows-1252'), 'R', 1, 'R', 0);
  $pdf->SetX(100);
}
$pdf->SetFont('Arial', 'B', 9);
$pdf->SetTextColor(255, 255, 255);
if (!isset($_GET['credit'])) {
  ...
  if ($row_orders['payment_status'] == 0) {
    ...
    if (strpos($row_orders['select_type'], 'entreprise') === false) {
      if ($row_orders['payment_status'] != 2) {
        $pdf->Cell(15, $step, mb_convert_encoding(number_format((($total_ht - $total_ht/100*$row_orders['transportation_time']) + ($total_tva - $total_tva/100*$row_orders['transportation_time']) - ($row_orders['deposit'] == 0 ? $row_orders['advance_payment'] : $row_orders['deposit'])), 2, ',', ''), 'windows-1252'), '', 0, 'R', 0);
      } else {
  ...
  $pdf->Cell(15, $step, mb_convert_encoding(number_format($_GET['credit']*1.2, 2, ',', ''), 'windows-1252'), 'RTB', 1, 'R', 1);
}
