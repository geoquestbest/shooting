    ...
    if (openPopupButtons) {
        ...
        openPopupButtons.forEach((button) => {
            button.addEventListener('click', (item) => {
                ...
                let popid = button.getAttribute('data-id');
                let popidx = button.getAttribute('data-idx');
                leafing(popid, popidx);
...
function leafing(popid, popidx) {
    let buttonPrev = document.querySelectorAll('.buttonPrev_' + popid);
    let buttonNext = document.querySelectorAll('.buttonNext_' + popid);
    let controlNumbor = +popidx;
    let popup = document.querySelectorAll('.' + popid);
    console.log(popid, buttonNext);
    if (buttonNext) {
        buttonNext.forEach((item) => {
            //item.removeEventListener('click', function () {});
            item.addEventListener('click', function () {
              console.log(controlNumbor);
                controlNumbor += 1;
